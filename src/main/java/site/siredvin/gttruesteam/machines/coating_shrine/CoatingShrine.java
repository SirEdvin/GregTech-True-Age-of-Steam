...
public class CoatingShrine {
    private static Supplier<BlockInfo[]> BLOCK_SUPPLIER = () -> new BlockInfo[]{BlockInfo.fromBlock(Blocks.STONE_BRICKS)};
    private static Supplier<BlockInfo[]> FLUID_SUPPLIER = () -> new BlockInfo[]{BlockInfo.fromBlock(Blocks.LAVA)};

    @SuppressWarnings("deprecation")
    public static MultiblockMachineDefinition MACHINE = GTTrueSteam.REGISTRATE.multiblock("coating_shrine", CoatingShrineMachine::new)
            ...
            .additionalDisplay(((iMultiController, components) -> {
                if (iMultiController.isFormed()) {
                    if (iMultiController instanceof CoatingShrineMachine coatingShrineMachine) {
                        ...
                        components.add(TrueSteamLang.COATING_CHARGES.append(" ").append(Integer.toString(logic.getCoatingCharges())));
                    }
                }
            }))
            .tooltips(
