...
- [x] 2.2 Implement the regular hatch part, server-tick observation, and front-only output; verify with a runtime fixture that idle formed controllers are sampled, OR output changes by the next tick, and detached controllers and unavailable values contribute zero.
