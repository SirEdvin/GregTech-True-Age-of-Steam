...
- [x] 2.2 Implement the regular hatch part, server-tick observation, and front-only output; verify with a runtime fixture that idle formed controllers are sampled, first-match output changes by the next tick, and detached/unsupported controllers emit zero.
...
- [x] 4.1 Build the type-aware ordered-rule editor with value/operator selection, operand input where appropriate, strength 0–15, add/delete, and up/down controls; verify all six rules fit or scroll at LuV and reorder changes actual server output priority.
...
- [x] 5.1 Run the focused automated suite covering every comparison/type and capacity tier, serialization corruption, unavailable values, and zero-strength priority; inspect actual test results and require nonzero executed test counts.
