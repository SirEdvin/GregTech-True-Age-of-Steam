...
Compact layout: each 40-pixel row puts the value selector, comparison, operand, output arrow/strength, Save, and Delete on one line. Shared Condition/Output headings replace repeated framing; the existing status occupies a slim unframed line below. Keep explicit saving and existing feedback semantics unchanged in this layout-only pass.
